...
internal sealed class ValidationPipelineBehavior<TResquest, TResponse>(
    IEnumerable<IValidator<TResquest>> validators)
    ...
    where TResquest : IBaseCommand
{
    ...
    private async Task<ValidationFailure[]> ValidateAsync(TResquest resquest)
    {
        if (!validators.Any())
        {
            return [];
        ...
        var context = new ValidationContext<TResquest>(resquest);
